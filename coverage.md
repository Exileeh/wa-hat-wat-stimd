...
| **Zuid-Holland, Fryslân, Gelderland, Overijssel** (Notubiz) | ⚠️ **CI-only failure** | first bot run, 2026-06-18 | Diagnosed 2026-09-13 from two GitHub runners (Azure eastus + westus): DNS and a control host are fine, but **TCP to `api.notubiz.nl:443` (195.20.144.5) times out** — packets silently dropped by a firewall before any HTTP happens, so a User-Agent exception cannot help — and the Cloudflare-fronted portal (`pzh.notubiz.nl`, needed for the per-fractie breakdown) answers **HTTP 403** in 30 ms. Both hosts filter this traffic; whether the rule targets cloud/datacenter IPs or non-EU locations is not yet known. Refresh locally until solved. |
